      ...
      if let mode = await xpsq8State.displacementMode(for: dimension) {
        if await mode != xpsq8State.lastSetDisplacementMode(for: dimension) {
          let parameters: Stage.SGammaParameters = {
            switch mode {
            case .large:
                return (10.0, 100.0, 0.2, 0.2)
            case .medium:
                return (1.0, 1.0, 0.2, 0.2)
            case .small:
                return (0.1, 5.0, 0.2, 0.2)
            case .fine:
                return (0.01, 1.0, 0.2, 0.2)
            }
          }()
          ...
          await setXPSQ8LastSetDisplacementMode(in: dimension, to: mode)
			...
			print("Printing: MoveAbsolute: \(dimension) to: \(location) at time: \(now)")
			
      try await writeStage(for: dimension).moveAbsolute(to: location)
			now = Date.now.formatted()
			print("Printing: MoveAbsolute Done at time: \(now)")
      ...
      if let mode = await xpsq8State.displacementMode(for: dimension) {
        if await mode != xpsq8State.lastSetDisplacementMode(for: dimension) {
          ...
          await setXPSQ8LastSetDisplacementMode(in: dimension, to: mode)
			...
			var now = Date.now.formatted()
			print("Printing: MoveRelative: \(dimension) by: \(displacement) at time: \(now)")
